...
import { faClock, faEllipsisV, faGear, faPen, faRightFromBracket, faTrash, faXmark } from '@fortawesome/free-solid-svg-icons'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { signOut } from 'next-auth/react'
import { ChevronRightIcon } from '@radix-ui/react-icons'
import * as Dialog from '@radix-ui/react-dialog'
import { useMutation } from '@apollo/client'
import { GetBlocksDocument, GetBlocksQuery, GetBlocksQueryVariables, UpdateBlockDocument, UpdateBlockMutation, UpdateBlockMutationVariables } from '../../../graphql/dist/client'

export function PartAndWorkDropdown({ handleDeleteWork, editHandler }: { handleDeleteWork: () => void; editHandler: () => void }) {
    return (
        <DropdownMenu.Root>
            <DropdownMenu.Trigger>
                <Button variant="soft" className="rounded-full p-0.5 hover:bg-gray-100">
            ...
            <DropdownMenu.Content className="z-40 w-40 rounded-xl bg-white text-xl shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px]">
                <DropdownMenu.Item className="m-1 flex items-center justify-between rounded-2xl px-7 py-1.5 text-center font-bold text-gray-600 hover:bg-gray-100 hover:outline-none" onClick={() => editHandler()}>
                    <FontAwesomeIcon icon={faPen} style={{ color: '#475569' }} />
                    <p>編集</p>
                </DropdownMenu.Item>
                <DropdownMenu.Item className="m-1 flex items-center justify-between rounded-2xl px-7 py-1.5 text-center font-bold text-gray-600 hover:bg-gray-100 hover:outline-none" onClick={() => handleDeleteWork()}>
...
export function UserDropdown({ userIconURL }: { userIconURL: string | null | undefined }) {
    return (
        <DropdownMenu.Root>
            <DropdownMenu.Trigger className="mx-5  flex select-none   items-center text-center text-gray-600">
                <img src={`${userIconURL}`} alt="userIcon" className="w-12 rounded-full  hover:opacity-70" />
            </DropdownMenu.Trigger>
            <DropdownMenu.Content className="z-10 w-40 rounded-xl bg-white text-xl shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px]">
                ...
                <DropdownMenu.Item className="m-1 flex items-center justify-between rounded-2xl px-7 py-1.5 text-center font-bold text-gray-600 hover:bg-gray-100 hover:outline-none" onClick={() => signOut()}>
...
export function BlockDropdown({ deleteBlock }: { deleteBlock: () => void }) {
    return (
        <DropdownMenu.Root>
            ...
            <DropdownMenu.Content className="z-10 w-40 rounded-xl bg-white text-xl shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px]">
                <DropdownMenu.Item className="m-1 flex items-center justify-between rounded-2xl px-7 py-1.5 text-center font-bold text-gray-600 hover:bg-gray-100 hover:outline-none" onClick={() => deleteBlock()}>
                    <FontAwesomeIcon icon={faTrash} style={{ color: '#f87171' }} />
                    <p className="text-red-400">削除</p>
                </DropdownMenu.Item>
            </DropdownMenu.Content>
...
const jpStandardSpeaker = ['ja-JP-Standard-A', 'ja-JP-Standard-B', 'ja-JP-Standard-C', 'ja-JP-Standard-D']
const usStandardSpeaker = ['en-US-Standard-A', 'en-US-Standard-B', 'en-US-Standard-C', 'en-US-Standard-D', 'en-US-Standard-E', 'en-US-Standard-F', 'en-US-Standard-G', 'en-US-Standard-H', 'en-US-Standard-I', 'en-US-Standard-J']

export function SpeakerDropDown({ block }: { block }) {
    const [speaker, setSpeaker] = useState(block.speaker)
    const [updateBlock] = useMutation<UpdateBlockMutation, UpdateBlockMutationVariables>(UpdateBlockDocument, {
        variables: { blockId: block.id, speaker },
        update(cache, { data }) {
            console.log(data)
            const newBlock = data!.updateBlock
            const partId = data!.updateBlock.partID
            cache.updateQuery<GetBlocksQuery, GetBlocksQueryVariables>(
                ...
                (result) => {
                    ...
                    return {
                        getPartById: {
                            id: partId,
                            blocks: result.getPartById.blocks?.filter((b) => (b.id === block.id ? newBlock : b)),
                        },
                    }
                }
            )
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger asChild>
                {duration !== 0 ? <button className="rounded-3xl border-2 border-teal-500 bg-white px-4 py-2 font-bold text-teal-500  opacity-0 group-hover:opacity-100">{duration}s</button> : <button className="rounded-3xl border-2 border-teal-500 bg-white px-4 py-2 font-bold text-teal-500  opacity-0 group-hover:opacity-100">間隔を指定</button>}
            </Dialog.Trigger>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="data-[state=open]:animate-contentShow fixed left-[50%] top-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
                    <Dialog.Title className="text-mauve12 m-0 text-[17px] font-bold text-gray-600">ブロック間の間隔を追加</Dialog.Title>
                    ...
                    <div>
                        ...
                        <div className="flex">
                            <input type="number" className="h-10 w-64 rounded-xl border-[2px] border-gray-200 focus:border-teal-500 focus:outline-none" onChange={(e) => setDuration(Number(e.target.value))} />
                            <Dialog.Close asChild>
                                <button className="mx-4 h-10 rounded-xl border-2 border-teal-500 bg-white px-4 py-2  font-bold text-teal-500">追加</button>
                            </Dialog.Close>
                        </div>
                    </div>

                    <Dialog.Close asChild>
                        <button className="text-violet11 hover:bg-violet4 focus:shadow-violet7 absolute right-[20px] top-[20px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:shadow-[0_0_0_2px] focus:outline-none" aria-label="Close">
