...
import { CreateBlockDocument, CreateBlockMutation, CreateBlockMutationVariables, GetBlocksDocument, GetBlocksQuery, GetBlocksQueryVariables } from '@/../graphql/dist/client'
import { useMutation, useQuery } from '@apollo/client'

function Page({ params }: { params: { workId: string; partId: string } }) {
    const { data } = useQuery<GetBlocksQuery, GetBlocksQueryVariables>(GetBlocksDocument, { variables: { partId: params.partId } })
    const blocks = data?.getPartById?.blocks

    const [createBlock] = useMutation<CreateBlockMutation, CreateBlockMutationVariables>(CreateBlockDocument, {
    ...
    return (
        <div className="mt-10 flex w-full  justify-center overflow-scroll bg-gray-50">
            <div className="w-3/6 ">
                <div className="">{blocks?.map((block) => <AtomBlock key={block.id} block={block} partId={params.partId} />)}</div>
